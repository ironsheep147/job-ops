...
): Pick<CareerOpsListing, "isRemote" | "remoteScope" | "eligibleCountryKeys"> {
  const value = location.toLowerCase();
  const isRemote =
    explicitRemote ||
    /remote|work from home|worldwide|anywhere|wfh/.test(value);
  if (!isRemote) return { isRemote: false };
  const restricted =
    /canada|europe|emea|asia|india|uk|united kingdom|australia|germany|france|latam|latin america|australia|new zealand/.test(
      value,
    ) && !/united states|usa|north america/.test(value);
  return {
    isRemote: true,
    remoteScope: restricted
      ...
      : value === "remote"
        ? "unspecified"
        : "unrestricted",
    eligibleCountryKeys: /united states|usa|north america/.test(value)
      ? ["united states"]
      : undefined,
  };
}
...
function listing(
  args: Partial<CareerOpsListing> &
    Pick<
      CareerOpsListing,
      "sourceJobId" | "title" | "employer" | "jobUrl" | "location"
    >,
): CareerOpsListing {
  const metadata = remoteMeta(args.location, args.isRemote);
  return {
    ...args,
    ...metadata,
...
async function fetchAshby(
  ...
  return (Array.isArray(payload?.jobs) ? payload.jobs : []).flatMap(
    (job: any) => {
      const jobUrl = safeUrl("ashby", job.jobUrl, true);
      if (!jobUrl || typeof job.title !== "string") return [];
      const location = [
        job.location,
        ...(job.secondaryLocations ?? []).map((x: any) => x?.location),
      ]
        .filter(Boolean)
        .join(" · ");
      return [
        listing({
          ...
          applicationLink: safeUrl("ashby", job.applyUrl, true) ?? jobUrl,
...
async function fetchLever(
  ...
  const slug = slugFrom(url);
  if (!slug) return [];
  const host = new URL(url).hostname.startsWith("jobs.eu.")
    ? "api.eu.lever.co"
    : "api.lever.co";
  ...
  return (Array.isArray(payload) ? payload : []).flatMap((job: any) => {
    ...
    return [
      listing({
        ...
        location,
        description: text(job.descriptionPlain),
        applicationLink: safeUrl("lever", job.applyUrl, true) ?? jobUrl,
...
async function fetchSmartRecruiters(
  ...
  for (let offset = 0; offset < 5000 && out.length < 40; offset += 100) {
    ...
    for (const row of rows) {
      const jobUrl =
        safeUrl("smartrecruiters", row.ref, true) ??
        `https://jobs.smartrecruiters.com/${slug}/${row.id}`;
    ...
    try {
      ...
      const description =
        ...
        Object.values(sections)
          .map((section: any) => text(section?.text))
          .filter(Boolean)
          .join("\n");
      if (description) row.description = description;
      row.applicationLink =
        safeUrl("smartrecruiters", detail?.jobAd?.applyUrl, true) ??
        row.applicationLink;
    } catch {
...
function parseCards(
  ...
  const out: CareerOpsListing[] = [];
  const cards = html
    .split(/(?:iCIMS_JobCardItem|data-job-id=|<item\b)/i)
    .slice(1);
  for (const card of cards) {
    const href = card.match(/href=["']([^"']+)["']/i)?.[1];
    const title = text(
      card.match(/<h[1-3][^>]*>([\s\S]*?)<\/h[1-3]>/i)?.[1] ??
        card.match(/<title[^>]*>([\s\S]*?)<\/title>/i)?.[1],
    );
    ...
    const location = text(
      card.match(
        /(?:Location|location|city)[^<:]*[:]?\s*<[^>]+>([\s\S]*?)<\//i,
      )?.[1] ?? card.match(/<(?:location|region)[^>]*>([\s\S]*?)<\//i)?.[1],
    );
    out.push(
      listing({ sourceJobId: jobUrl, title, employer, jobUrl, location }),
    );
    if (out.length >= 40) break;
  }
  return out;
...
async function fetchIcims(
  ...
  for (let page = 0; page < 3 && out.length < 40; page += 1) {
    const html = await request(
      `${parsed.origin}/jobs/search?ss=1&pr=${page}&in_iframe=1`,
      "text",
      signal,
    );
...
async function fetchWorkable(
  ...
  let payload: any;
  try {
    payload = await retry(() => request(api, "json", signal), signal);
  } catch {
    payload = null;
  }
  ...
  return rows.slice(0, 40).flatMap((job: any) => {
    ...
    return [
      listing({
        sourceJobId: String(job.shortcode ?? jobUrl),
...
async function fetchTeamtailor(
  url: string,
  employer: string,
  signal?: AbortSignal,
): Promise<CareerOpsListing[]> {
  ...
  const xml = await request(
    `https://${parsed.hostname}/jobs.rss`,
    "text",
    signal,
  );
  return (xml.match(/<item\b[\s\S]*?<\/item>/gi) ?? [])
    ...
    .flatMap((item: string) => {
      ...
      const location =
        [tag("tt:city"), tag("tt:country")].filter(Boolean).join(", ") ||
        (tag("remoteStatus") ? "Remote" : "");
      return [
        listing({
          sourceJobId: jobUrl,
          title,
...
async function fetchJobvite(
  ...
  if (!eid && parsed.hostname === "jobs.jobvite.com") {
    const html = await request(
      `${parsed.origin}${parsed.pathname}?fr=true&nl=1`,
      "text",
      signal,
    );
    eid =
      html.match(/companyEId\s*[:=]\s*["']([A-Za-z0-9_-]{4,40})["']/)?.[1] ??
      null;
  }
  if (!eid) return [];
  const feed = `https://app.jobvite.com/CompanyJobs/Xml.aspx?c=${encodeURIComponent(eid)}`;
  const xml = await request(feed, "text", signal);
  return (xml.match(/<job>[\s\S]*?<\/job>/gi) ?? [])
    .slice(0, 40)
    .flatMap((job: string) => {
      const tag = (name: string) =>
        text(
          job.match(new RegExp(`<${name}>([\\s\\S]*?)</${name}>`, "i"))?.[1],
        );
      ...
      return [
        listing({
          sourceJobId: tag("id") || jobUrl,
          title,
          employer,
          jobUrl,
          location: tag("location"),
          applicationLink: safeUrl("jobvite", tag("apply-url"), true) ?? jobUrl,
...
function mapGenericRows(
  ...
  return rows.slice(0, 40).flatMap((row) => {
    ...
    const jobUrl = safeUrl(
      provider,
      row?.url ?? row?.jobUrl ?? row?.applyUrl ?? row?.hostedUrl,
      true,
    );
    if (!title || !jobUrl) return [];
    const location = text(
      row?.location ?? row?.city ?? row?.locations ?? row?.address,
    );
    return [
      listing({
        sourceJobId: String(
          row?.id ?? row?.requisitionId ?? row?.slug ?? jobUrl,
        ),
        title,
        employer: text(row?.company ?? row?.companyName) || employer,
        jobUrl,
        applicationLink:
          safeUrl(provider, row?.applyUrl ?? row?.applicationUrl, true) ??
          jobUrl,
        location,
        description: text(
          row?.description ?? row?.descriptionPlain ?? row?.jobDescription,
        ),
...
async function fetchEnterprise(
  ...
  if (provider === "rippling" && parsed.hostname === "ats.rippling.com" && slug)
    endpoint = `https://api.rippling.com/platform/api/ats/v1/board/${encodeURIComponent(slug)}/jobs`;
  if (provider === "oraclecloud")
    endpoint = `${parsed.origin}/hcmRestApi/resources/latest/recruitingCEJobRequisitions`;
  if (provider === "phenom") endpoint = `${parsed.origin}/widgets`;
  if (provider === "avature")
    endpoint = `${parsed.origin}/careers/SearchJobs?jobOffset=0`;
  if (provider === "radancy") endpoint = `${parsed.origin}/en/search-jobs?p=1`;
  if (provider === "successfactors")
    endpoint = `${parsed.origin}/tile-search-results/?startrow=0`;
  ...
  if (typeof response === "string")
    return parseCards(response, provider, employer, parsed.origin);
  return mapGenericRows(provider, rowsFromPayload(response), employer);
}

export async function fetchCareerOpsListings(
  provider: CareerOpsProvider,
  careersUrl: string,
  employer: string,
  signal?: AbortSignal,
  ...
  switch (provider) {
    ...
    case "lever":
      return fetchLever(careersUrl, employer, signal);
    case "smartrecruiters":
      return fetchSmartRecruiters(careersUrl, employer, signal);
    case "icims":
      return fetchIcims(careersUrl, employer, signal);
    case "workable":
      return fetchWorkable(careersUrl, employer, signal);
    case "teamtailor":
      return fetchTeamtailor(careersUrl, employer, signal);
    case "jobvite":
      return fetchJobvite(careersUrl, employer, signal);
    case "eightfold":
    case "oraclecloud":
    case "phenom":
    case "avature":
...
export function toWatchlistJob(
  provider: CareerOpsProvider,
  row: CareerOpsListing,
): Omit<
  WatchlistJobResult,
