...
const URL = "https://speedrun-talent-network.com/api/v1/jobs";

export async function runA16zspeedrun(context: ExtractorRuntimeContext) {
  const options = commonOptions(context);
  if (options.cancelled()) return { success: true, jobs: [] };
  options.progress("a16zspeedrun: fetching listings");
  try {
    ...
    for (const row of rows) {
      ...
      const job = genericJob("a16zspeedrun", row);
