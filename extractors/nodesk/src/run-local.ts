...
const URL = "https://nodesk.co/remote-jobs/index.xml";

export async function runNodesk(context: ExtractorRuntimeContext) {
  const options = commonOptions(context);
  if (options.cancelled()) return { success: true, jobs: [] };
  options.progress("nodesk: fetching listings");
  try {
    ...
    for (const row of rows) {
      ...
      const job = genericJob("nodesk", row);
